...
<div class="container">
    ...
    <div style="text-align: right; margin-bottom: 20px;">
        <a href="{{ route('admin.reviews.create') }}" class="btn-add">商品レビュー登録</a>
    </div>
        ...
        <form method="GET" action="{{ route('admin.reviews.index') }}">
            <div class="search-row">
                ...
                <div class="search-item search-keyword">
                    <label>フリーワード</label>
                    <input type="text" name="freeword" value="{{ $freeword }}" placeholder="コメント">
                </div>
            </div>
        ...
        <table class="data-table">
            <thead>
                <tr>
                    <th class="col-id">
                        ID
                        <a href="{{ route('admin.reviews.index', array_merge(request()->except(['sort', 'direction']), ['sort' => 'id', 'direction' => $sortColumn === 'id' && $sortDirection === 'asc' ? 'desc' : 'asc'])) }}" class="sort-btn">
                            @if($sortColumn === 'id')
                                {{ $sortDirection === 'asc' ? '▲' : '▼' }}
                            @else
                                ▼
                    ...
                    <th class="col-comment">商品コメント</th>
                    <th class="col-date">
                        登録日時
                        <a href="{{ route('admin.reviews.index', array_merge(request()->except(['sort', 'direction']), ['sort' => 'created_at', 'direction' => $sortColumn === 'created_at' && $sortDirection === 'asc' ? 'desc' : 'asc'])) }}" class="sort-btn">
            ...
            <tbody>
                ...
                @empty
                    <tr>
                        <td colspan="6" class="no-data">該当するレビューが見つかりませんでした</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
